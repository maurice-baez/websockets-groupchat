...
const JOKE_API_URL = "http://icanhazdadjoke.com/";
...
class ChatUser {
  ...
  async handleJoke() {
    const response = await axios.get(JOKE_API_URL,
      { headers: { Accept: "application/json", "user-agent": "websocket-groupchat" } });

    const joke = {type: "joke", joke: response.data.joke};
    this._send(JSON.stringify(joke));
  }
